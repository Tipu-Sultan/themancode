  ...
  const AuthenticateUser = async e => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        const getresponse = await axios.get(`${process.env.REACT_APP_API_HOST}/api/auth/profile`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
        if (getresponse.status === 200) {
          localStorage.setItem("isLogin", JSON.stringify(getresponse.data));
      ...
      toast.error(error.response?.data.error || 'ERR_CONNECTION_REFUSED');
      setWait(false);
    }
  };
